...
: SPACE.TO.COLUMN  ( col -- ) out @ - spaces ;
: IS.PRIMITIVE? ( xt -- flag , true if kernel primitive )
    ['] first_colon < ;

0 value TRACE-BEGWORD_IP \ Instruction pointer begining of words
...
: TRACE.(LOCAL.ENTRY)  ( x0 x1 ... xn n -- )  { num | lp -- nothink }
    trace-locals-ptr @ trace.>r
    trace-rsp @ trace-locals-ptr !
    trace-rsp @  num cells - trace-rsp !  \ make room for locals
    trace-rsp @ -> lp
    num 0
    DO lp ! cell +-> lp  \ move data into locals frame on return stack
    LOOP ;
...
: TRACE.CHECK.IP  {  ip -- }
    ...
    ip here u> OR
    IF
        ." TRACE - IP out of range = " ip .hex cr abort
    THEN ;

: TRACE.SHOW.IP { ip -- , print name and offset }
    ip ( >code code> ) ( code@ ) name> >name dup id.
    name> ip code> swap - 4 / ."  +" .dec ." 's words" ;
...
\ show word name and offset
    ." <<  "
    trace_ip trace.show.ip
    30 space.to.column
\ show data stack
    trace.show.stack
    80 space.to.column ." ||   Next word - "
...
\ trap any primitives that are followed by inline data
    CASE
        ['] (LITERAL)  OF ip @ dup . space 255 and emit ENDOF
        ['] (ALITERAL) OF ip a@ dup . space 255 and emit  ENDOF
[ exists? (FLITERAL) [IF] ]
        ['] (FLITERAL) OF ip f@ f. ENDOF
[ [THEN] ]
        ['] BRANCH     OF ip @ 4 / . ENDOF
        ['] 0BRANCH    OF ip @ 4 / . ENDOF
...
: gd { more_levels | stop_level userflag -- }
...
: g ( -- , execute until end of word ) 0 gd ;
: TRACE.HELP ( -- )
...
0 [IF]
variable var1 100 var1 !
: FOO  dup IF 1 + . THEN 77 var1 @ + . ;
: ZOO 29 foo 99 22 + . ;
: ROO 92 >r 1 r@ + . r> . ;
: MOO c" hello" count type ." This is a message." cr s" another message" type cr ;
: KOO 7 FOO ." DONE" ;
: TR.DO  4 0 DO i . LOOP ;
: TR.?DO  0 ?DO i . LOOP ;
: TR.LOC1 { aa bb } aa bb + . ;
